...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		 ...
		 * EXERCISE: Your code here */
		 ...
		 // calculate size of directory (multiply size of directory entry with the number of files)
		 uint32_t size = dir_oi->oi_size * OSPFS_DIRENTRY_SIZE;

		 // since f_pos is offset by 2, it is subtracted by 1 to compare with total bytes
		 if (f_pos - 1 > size) 	
		 {
			r = 1;		/* Fix me! */
			break;		/* Fix me! */
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
	...
	// if no more links and the file isn't a symlink, then delete it
...
allocate_block(void)
{
	...
	// free-block bitmap starts at Block 2
	void * bitmap = ospfs_block(OSPFS_FREEMAP_BLK);

	// starting point
	uint32_t current = 0;

	// iterate through the block bitmap
	for (; current < num_blocks; current++)
	{
		// if the value of the bitmap for the current block is 1, then the block is free
		if (bitvector_test(bitmap, current))
		{
			// give it a value 0 in order to mark it as allocated
			bitvector_clear(bitmap, current);
			return current;
		}
	}
	return 0;
